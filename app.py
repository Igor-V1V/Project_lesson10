...
def transcribe_audio_with_openai(client: OpenAI, wav_path: str) -> str:
    ...
    raise RuntimeError(
        f"STT failed for all models. Last error: {last_err}"
    )
...
def diarize_by_llm(client: OpenAI, raw_transcript: str) -> str:
                ...
                messages=[
                    {
                        ...
                        "content": (
                            ...
                            "НЕ меняй слова.\n"
                            "Только разделяй реплики и добавляй "
                            "«Спикер 1», «Спикер 2».\n"
                            "Выводи только диалог."
                        ),
                    },
                    {
                        "role": "user",
    ...
    for s in sents:
        result.append(f"Спикер {speaker}: {s}")
...
async def analyze(request: Request):
    ...
    if upload:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            raw = transcribe_audio_with_openai(
                client,
                wav,
            )
